...
##Here is the data_augmentation method we proposed
class FadeInOutPerturbation(Perturbation):
    def __init__(self, sr, n_segments=4, segment_ratio=0.03, rng=None):
        self._sr = sr
    ...
    def perturb(self, data):
        n_samples = len(data._samples)
        segment_length = int(self._segment_ratio * n_samples)

        # Create a copy of the original samples
        perturbed_samples = np.copy(data._samples)

        # For each segment
        for i in range(self._n_segments):
            # Choose random start index for segment
            segment_start = self._rng.randint(0, n_samples - segment_length)
            segment_end = segment_start + segment_length
            
            # Split the segment into two halves
            mid_point = (segment_start + segment_end) // 2
            
            # Compute the fade-out and fade-in profiles
            fade_out = np.linspace(1.0, 0.0, mid_point - segment_start)
            fade_in = np.linspace(0.0, 1.0, segment_end - mid_point)
            
            # Apply fade out to the first half of the segment
            perturbed_samples[segment_start:mid_point] *= fade_out
            
            # Apply fade in to the second half of the segment
            perturbed_samples[mid_point:segment_end] *= fade_in
        
        # Compute spectrogram of the original audio
        original_spec = librosa.amplitude_to_db(np.abs(librosa.stft(data._samples)), ref=np.max)
        
        # Compute spectrogram of the perturbed audio
        perturbed_spec = librosa.amplitude_to_db(np.abs(librosa.stft(perturbed_samples)), ref=np.max)
...
perturbation_types = {
    ...
    "fade_in_out": FadeInOutPerturbation,
